 ...
 */
function getQQLevelInfo(id) {
  let res = fetchApi('GET', `https://club.vip.qq.com/api/vip/getQQLevelInfo?requestBody={"iUin":${id}}`)
...
function getGroupSetting(id) {
  let res = fetchApi('GET', `https://qinfo.clt.qq.com/cgi-bin/qun_info/get_group_setting_v2?gc=${id}&bkn=${BKN}`)
  return JSON.parse(res)
}
 ...
 * @returns
 */
function setAnonySwitch(id, isAnony) {
  isAnony = isAnony ? 1 : 0
  return fetchApi(
    'GET',
    `https://qqweb.qq.com/c/anonymoustalk/set_anony_switch?bkn=${BKN}&value=${isAnony}&group_code=${id}`
  )
}
...
function getFriendInfo(id) {
  let res = fetchApi('GET', `https://cgi.find.qq.com/qqfind/buddy/search_v3?keyword=${id}`)
  return JSON.parse(res)
}
