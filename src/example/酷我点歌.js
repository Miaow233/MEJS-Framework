...
$.on('message', async (session) => {
  ...
  function get_cookie() {
    let [res, data] = Http.get("http://www.kuwo.cn/", null, {
      headers = {
        Connection: 'keep-alive',
        Host: 'www.kuwo.cn',
  ...
  function get_music_list(cookie, key, pn, length) {
    ...
    for (let [i, v] in Object.entries(params)) {
      base_url = base_url + i + "=" + v + "&"
    }
    ...
    if (string.len(res) > 88) {
      ...
      for (let [i, v] in Object.entries(music_list)) {
        ...
        let pic = v['pic']
        let data = string.format('{"name": "%s", "artist": "%s", "rid": %d ,"pic":"%s"}', name, artist, rid, pic)
  ...
  //         cookie = get_cookie()
  //         music_data = get_music_list(cookie, music_name, 1,list_length)
  //         let data = Json.parseJson(music_data[1])
  //         music_rid = data.rid
  //         music_artist = data.artist
  //         music_name = data.name
  //         music_picurl = data.pic
