...
export class MineSweeper {
  ...
  static #get_count_text_color(count): string {
    ...
    if (count == 4) return //ImageColor.getrgb("darkred")
  ...
  #is_valid_location(row: number, column: number): boolean {
    if (row > this.row - 1 || column > this.column - 1 || row < 0 || column < 0)
      return false
    return true
  }
  ...
  draw_panel() {
    ...
    console.log(`draw spend ${new Date().getTime() - start}ms at {str(this)}`)
    return this.img
  }
  ...
  mine(row: number, column: number) {
    ...
    this.#win_check()
    console.log(
      `mine spend ${new Date().getTime() - start}ms at ${String(this.panel)}`
    )
  }
  ...
  tag(row: number, column: number) {
    ...
    console.log(
      `tag spend ${new Date().getTime() - start}ms at ${String(this)}`
    )
  }
  ...
  #gen_mine() {
    ...
    while (count < this.mines) {
      ...
      if (this.panel[row][column].is_mine || this.panel[row][column].is_mined)
        continue
  ...
  #spread_not_mine(row: number, column) {
